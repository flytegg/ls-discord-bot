...
class GiveawayManager(private val bot: JDA, private val datastore: Datastore) {
    ...
    private val giveawayMessages: MutableMap<String, ScheduledFuture<*>> = mutableMapOf()
    ...
    init {
        ...
        datastore.database.getCollection("giveaways").find().forEach {
            document ->
        ...
        bot.listener<ButtonInteractionEvent> {
            if(it.componentId == "enter-giveaway"){
                ...
                val giveaway = datastore.findGiveaway(message.id)
                it.hook.editOriginal("You are now " +
                ...
                message.editMessageEmbeds(buildEmbed(false, host, giveaway.prize,
    ...
    private fun endGiveaway(giveaway: Giveaway) {
        ...
        if(participants.size == 0) {
            ...
            for (i in 1..giveaway.winnerAmount) {
                val randomWinner = Random(Instant.now().toEpochMilli()).nextInt(0, participants.size - 1)
                winners.add(participants[randomWinner])
            }
        }
        val winnersString = winners.stream().map { id -> bot.retrieveUserById(id).complete().asMention }.collect(Collectors.joining(", "))
    ...
    private fun buildEmbed(ended: Boolean, host: User, prize: String, endTime: Long, winnerAmount: Int, entryAmount: Int, winnerString: String = ""): MessageEmbed {
