...
class AddReputationCommand {
    ...
    fun onManageRepAddCommand(
        ...
        @Description("Channel the reputation is being added from") @Optional @Named("from-channel") fromChannel: ChannelInput?,
        ...
        profile.addReputation(user, fromUser?.id ?: event.user.id, fromChannel?.channel?.id ?: event.channel.id, amount ?: 1)
        event.replyEmbeds(
            embed()
